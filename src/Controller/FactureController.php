...
{
    #[Route('/dashboard/facture', name: 'app_facture')]
    ...
    public function print(Invoice $invoice, Request $request): Response
    {
        ...
        // Récupérer le paramètre 'cachet' de la requête
        $cachet = $request->query->get('cachet', 'non'); // Par défaut, 'non' si non spécifié

        return $this->render('facture/print.html.twig', [
            'invoice' => $invoice,
            'amountInWords' => $amountInWords,
            'cachet' => $cachet, // Envoyer le paramètre 'cachet' au template
        ]);
    }
    ...
    public function pay(int $id, Request $req, EntityManagerInterface $em, AccountTransactionRepository $repo): JsonResponse
    {
        ...
        $em->persist($tx);
         ...
         if ($client->getType() === 'gesta' && $client->getCommittee() !== null && $client->getCommittee() !== '') {
            ...
            $txapp = new AccountTransaction();
            $txapp->setInvoice($inv)  // présuppose relation Invoice dans Transaction
                ->setIncome(0)
                ->setOutcome($amount * 10/100)
                ->setAccountType('supplier')
                ->setReason('Commission de facture')
                ->setUpdatedAt(new \DateTimeImmutable())
                ->setDescrib('Retrait de la commmission #'.$inv->getRef())
                ->setBalanceValue($newBalance_supplier - $amount * 10/100)
            ...
            $newBalance_expense = $balance_expense + $amount * 10/100;

            $txexp = new AccountTransaction();
            $txexp->setInvoice($inv)  // présuppose relation Invoice dans Transaction
                ->setIncome($amount * 10/100)
                ->setOutcome(0)
                ->setAccountType('expense')
                ->setReason('Commission de facture')
                ->setUpdatedAt(new \DateTimeImmutable())
                ->setDescrib('Débit de la commmission #'.$inv->getRef())
                ->setBalanceValue($newBalance_expense)
                ->setPaymentMethod('Transfert depuis approvisionnement') 
                ->setPaymentRef($data['paymentReference'] ?? '')
                ->setStatus('en attente')
                ->setUser($this->getUser()) 
                ->setCreatedAt(new \DateTimeImmutable());
