...
use ContainerDidY1aD\getAccountTransactionService;
use DateTime;
...
final class DashboardController extends AbstractController
{
    ...
    public function getStats(
        ...
        $response['stats']['capitalCFA'] = [
            ...
            'secondary' => '+50000 aujourd\'hui' // Placeholder
        ];
        ...
        $response['Transfers'] = array_map(fn($t) => [
            'id' => $t->getId(),
            'reference' => $t->getRef(),
    ...
    #[Route("/api/localbalance/", name: 'getlocalbalance', methods:['GET'])]
    public function getLocalBalance(AccountTransactionRepository $repo, AgenceRepository $agenceRepo): JsonResponse
    {
        $agence = $agenceRepo->FindOneBy(['id' => 1]);
        $capitalCFA = $repo->createQueryBuilder('t')
            ->select('COALESCE(SUM(t.CFA), 0)')
            ->where('t.agence = :agence')
            ->setParameter('agence', $agence)
            ->getQuery()
        ...
        return $this->json(['capital' => $capitalCFA]);
    }
